...
class ReceiptForm(FlaskForm):
    ...
    identifier = StringField(
        u'编号', validators=[DataRequired(message=u'编号不能为空')])
    maker = StringField(u'制单人', validators=[DataRequired(message=u'制单人不能为空')])
    repository = SelectField(u'仓库', validators=[DataRequired()], choices=[
                             ('主仓库', '主仓库'), ('次仓库', '次仓库')])
    balance = SelectField(u'结算方式', validators=[DataRequired()], choices=[
                          ('货到付款', '货到付款'), ('款到发货', '款到发货')])
